...
package:
	cd .. && rm -f *.zip && zip -r quicksaveqml.experimental.zip quicksaveqml -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\*  \*.svn\* \*Makefile*
	mv ../quicksaveqml.experimental.zip .

upload:
	plugin_uploader_NG.py quicksaveqml.experimental.zip
